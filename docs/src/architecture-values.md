...
Maintainability composes with Simplicity (fewer mechanisms are easier to maintain) and with Reversibility (a fix you can revert minimally is one whose blast radius is bounded). It trades off against premature generality — the discipline is *not* "abstract everything"; it is "abstract on the third occurrence, when the abstraction deepens rather than thinning." Duplication is far cheaper than the wrong abstraction; the rule-of-three is the extraction trigger and the depth test is the gate.
