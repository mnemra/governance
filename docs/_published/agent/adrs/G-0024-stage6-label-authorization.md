...
G-0013 locked the closed-enum label vocabulary as the canonical Stage 6 approval signal. Round 1 security review surfaced a structural authorization gap: G-0013 specifies *what* the labels are and *what* the gate logic does, but not *who* may apply them or *what* prevents the GitHub merge UI from bypassing the gate logic entirely.

By GitHub default, any collaborator with the **Triage** role or higher can apply labels via the web UI or `gh api`. Combined with the absence of a required-status-check on the protected branch that depends on label cardinality validation, the workflow's Stage 6 gate is mechanically bypassable in two ways:
...
**Ruleset enforcement mechanism:** GitHub's ruleset API supports `actor_id` allowlist on the `pull_request` and `repository` rule types via custom labels rules where available, OR — when the platform tier doesn't expose this — a GitHub Actions workflow `verify-stage6-labeler` triggered on `pull_request.labeled` event:
...
Per-repo retrofit /spec implements one of these, depending on plan tier (verify availability via `gh api repos/{owner}/{repo}` + ruleset endpoint).
...
Branch-protection settings include **both** `verify-stage6-labels` AND `verify-stage6-labeler` in `required_status_checks.contexts`. Both checks are required-status-checks; a failing labeler check (Layer A) AND a failing cardinality check (Layer B) each block the merge. Putting Layer A in `required_status_checks` is what makes it preventive rather than detective — a spoofed-labeler PR with valid cardinality would still merge if Layer A is only a status check that's not listed in required-status-checks. Listing Layer A as required-status-check closes that gap.

**Layer A fail-open behavior under Actions out-of-quota:** if the GitHub Actions quota is exhausted, `verify-stage6-labeler` won't run, and `required_status_checks` treats a missing required check as "blocked" — the PR cannot merge. This is fail-closed, not fail-open. Document this in the per-repo retrofit runbook so operators don't mistake quota-exhaustion for a workflow bug.

**Cardinality-failure mode:** ABORT, never pick. The check fails loudly with stderr message identifying the conflicting labels and the cardinality rule violated. The PR's check status reads "failure" until the labels are corrected.

### Layer C — Audit cross-reference at retrieval time (R-pending-2 mitigation)

Audit row in the workspace activity log carries the actor as a CLI argument; this is self-attestation and cannot survive a credential compromise. Mitigation per accepted-risk R-pending-2:
...
**Eventual-consistency note.** GitHub's `/issues/<pr#>/events` endpoint reflects label changes within a few seconds of the underlying event. Audit reconstruction MUST query the event log only after the PR's `merged_at` timestamp + a small safety margin (recommend 60 seconds) to avoid catching mid-flight inconsistency.
...
- **Cryptographic actor binding via OIDC subject in audit row.** Rejected for v1. Real implementation work; doesn't fit "no new tools" simplicity anchor; mnemra-content-plugin can do this later. Layer C cross-reference is the lean substitute.
...
- **Pull request review approvals as the gate signal** (drop labels entirely; use GitHub's native review-approvals UI). Rejected. Locked in G-0013 alternatives — the closed-enum label vocabulary is the canonical signal for downstream tooling. Reverting to review-approvals re-opens settled architecture.
- **Workspace-side check via webhook → external service.** Rejected. Adds infrastructure; GitHub-native rulesets + Actions cover the gate without external dependencies.

## Consequences

- **Per-repo retrofit:** add the two workflows (`verify-stage6-labeler.yml` + `verify-stage6-labels.yml`); update branch protection to require **both** `verify-stage6-labeler` AND `verify-stage6-labels` in `required_status_checks.contexts`; configure repository ruleset for label-set permission. Deployment specialist dispatch per repo as part of the merge-mode migration runbook.
- **G-0013 references G-0024 for enforcement mechanism.** G-0013 amendment adds: "Cardinality enforcement and label-set authorization are specified in G-0024."
- **G-0018 references G-0024 for label-set authority.** Workspace template script sets the labels under the automation account's identity; the labels propagate through Layer A's check (automation accounts in allowlist).
...
- **GitHub plan tier dependency:** Layer A's repository ruleset mechanism varies by plan tier. The Actions-based fallback (workflow checking `github.event.sender.login` against an allowlist) works on all tiers and is the v1 default.
- **Allowlist maintenance:** v1 allowlist is the named maintainer plus designated automation accounts. Multi-collaborator state extends the allowlist via a separate /spec.
- **External-PR contributors:** out of scope for v1. In-scope repos restrict PR creation to collaborators. If/when external PRs enter scope, this ADR is amended with `pull_request_target` policy and fork-PR label-handling.
- **Mnemra migration:** GitHub-native primitives (rulesets, branch protection, Actions workflows) survive mnemra absorption unchanged. The allowlist's substrate may shift if mnemra introduces a richer identity model, but the pattern (named approvers gate the labels; cardinality is required-status-check) holds.
- **Audit cross-reference cost at retrieval time:** an extra `gh api` call per audit reconstruction. Acceptable cost — audit retrieval is human-initiated, not a hot path.
- **Bot allowlist by-name caveat (R-pending-5).** The Actions-based Layer A fallback enforces by `github.event.sender.login`, which is sender identity not authentication context. A compromised credential for an automation account can apply gating labels through Layer A's allowlist. Mitigations: fine-grained credentials scoped to label-write permission only, 90-day rotation, GitHub App tokens (preferred when feasible per G-0020 token strategy). Captured as R-pending-5 in the constraints summary's accepted-risks table.

## Changelog

### 2026-04-30 — Round 2 review amendments

- **Layer A detective-only gap:** original draft listed only `verify-stage6-labels` in `required_status_checks.contexts`. Spoofed-labeler PRs with valid cardinality would still merge under Layer B alone. **Fixed:** both `verify-stage6-labeler` AND `verify-stage6-labels` now required-status-checks.
- **Layer C narrow:** Layer C cross-reference covered `stage6-approved` only; bump and release-mode label values weren't queried, AND the audit-row action string didn't carry the bump value. **Fixed:** widened action-string format to include `bump <Y>` and widened the GitHub event log query to all three label families.
- **Layer A out-of-quota fail-open concern:** verified fail-closed: `required_status_checks` treats missing required check as blocked; quota exhaustion blocks merges, doesn't unblock them. Added explanatory paragraph + retrofit-runbook note.
