...
G-0006 (a governance-level architecture decision record) fixed `just ci` as the CI contract and named devcontainer integration as adjacent, deferring it to a follow-up ADR. This is that follow-up.
...
Two forces point the same way. The QA anchor here is the simplicity quality attribute (a non-functional property that applies to every project regardless of feature scope), and it favors per-repo independence. So does forward compatibility: the multi-repo polyglot model wants each repo to own its environment without depending on a workspace-side build pipeline that may not exist after mnemra absorbs the workspace.
...
**Layered binary pinning is mandatory.** Every `cargo install` line in a devcontainer Dockerfile MUST pass both `--locked` and `--version <X.Y.Z>`. `--locked` protects against transitive churn for that install invocation; `--version` pins the top-level binary, closing the supply-chain window where a future patch release of a devcontainer tool could be baked into a privileged-context image. Dependabot's Dockerfile parser tracks pinned `cargo install --version` lines, so the per-repo retrofit's Dependabot config MUST include Dockerfile pattern coverage so binary-version bumps surface as PRs. Once ecosystem tools ship signed release artifacts, the stretch goal is to switch to `cargo binstall` with checksum verification.
...
The repo's CI workflow includes a job that, on changes to `.devcontainer/**`, builds the image and pushes to `ghcr.io/<owner>/<repo>-devcontainer:<git-sha>` plus `:latest`. CI runs (`just ci`) execute inside this image. The image is pulled, not rebuilt, so local-vs-CI parity holds.
...
- **`devcontainers/cli` build-on-demand without GHCR push.** Rejected. Pulling and rebuilding per CI run wastes time and bandwidth; pushing the built image to GHCR makes CI a pull-only operation.
...
- **Per-repo CI workflow includes a devcontainer-publish job.** Triggered on `.devcontainer/**` path filter. Workspace template ships this workflow YAML as `devcontainer-publish.yml`. Required permissions on the calling job: `contents: read, packages: write, id-token: write, attestations: write` (packages:write for GHCR push, id-token:write and attestations:write for OIDC-based provenance attestations). Workflow uses `docker/login-action` against `ghcr.io`, `docker/setup-buildx-action`, and `docker/build-push-action`. **Multi-arch posture:** SHA-pin targets the upstream manifest list (the multi-arch manifest); per-repo builds run on `ubuntu-latest` amd64 only by default. An arm64 build matrix is added per-repo via `/spec` (the spec workflow that defines what done looks like before implementation) when an arm64 contributor or runner enters scope. It isn't a workspace default, which avoids 2x build cost without demand.
...
- **Retrofit cost per repo is about 1 PR:** `.devcontainer/Dockerfile`, `devcontainer.json`, `.github/workflows/devcontainer.yml`, and the Dependabot config update. The deployment specialist (the operations role) is the dispatch target per repo.
...
- **Added:** Layered binary pinning requirement. Every `cargo install` line in a devcontainer Dockerfile MUST pass `--locked --version <X.Y.Z>`. Dependabot Dockerfile pattern coverage required in per-repo retrofit. cargo-binstall plus signed releases noted as a stretch goal once upstreams support it.
